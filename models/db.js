...
async function getUserByEmail(email, role = "ambassador") {
  try {
    ...
    if (roleError || !roleData) {
      console.error(
        `❌ No ${role} profile found for user_id:`,
        userData.user_id
      );
      return null;
    }
...
async function getUserById(id, role = "ambassador") {
  try {
    ...
    if (roleError || !roleData) {
      console.log(
        `⚠️ No ${role} found with user_id: ${id}, trying ${roleIdField}...`
      );
    ...
    if (userError || !userData) {
      console.error(
        `❌ No user found for ${role} with user_id:`,
        roleData.user_id
      );
      return null;
    }
  ...
  try {
    ...
    if (role === "ambassador") {
      ...
      roleInsert = {
        user_id: newUser.user_id,
        organization_name:
          userData.organization_name || userData.organizationName || "",
...
async function updateUser(id, updates, role = "ambassador") {
  try {
    ...
    if (updates.password_hash !== undefined)
      userUpdates.password_hash = updates.password_hash;
    ...
    if (updates.access_code !== undefined)
      userUpdates.access_code = updates.access_code;
    if (updates.last_login !== undefined)
      userUpdates.last_login = updates.last_login;

    // Fields that go to role-specific tables
    if (role === "ambassador") {
      if (updates.first_name !== undefined)
        roleUpdates.first_name = updates.first_name;
      if (updates.last_name !== undefined)
        roleUpdates.last_name = updates.last_name;
      if (updates.cv_filename !== undefined)
        roleUpdates.cv_filename = updates.cv_filename;
      if (updates.gender !== undefined) roleUpdates.gender = updates.gender;
      if (updates.whatsapp_number !== undefined)
        roleUpdates.whatsapp_number = updates.whatsapp_number;
      ...
      if (updates.continent !== undefined)
        roleUpdates.continent = updates.continent;
    } else if (role === "partner") {
      if (updates.organization_name !== undefined)
        roleUpdates.organization_name = updates.organization_name;
      if (updates.contact_person !== undefined)
        roleUpdates.contact_person = updates.contact_person;
      if (updates.phone_number !== undefined)
        roleUpdates.phone_number = updates.phone_number;
      if (updates.location !== undefined)
        roleUpdates.location = updates.location;
      if (updates.partner_type !== undefined)
        roleUpdates.partner_type = updates.partner_type;
    } else if (role === "admin") {
      if (updates.first_name !== undefined)
        roleUpdates.first_name = updates.first_name;
    }
...
async function listUsers(role = "ambassador", filters = {}) {
  try {
    ...
    let query = supabase.from(roleTable).select(
      `
        *,
        users!inner(*)
      `,
...
// ============================================
// NEW JOURNEY SYSTEM FUNCTIONS (Database-based)
// ============================================

// Get all journey months
async function getJourneyMonths() {
  try {
    const { data, error } = await supabase
      .from("journey_months")
      .select("*")
    ...
    console.error("❌ getJourneyMonths error:", error);
    return [];
  }
}

// Get tasks for a specific month
async function getJourneyTasksByMonth(monthId) {
  try {
    const { data, error } = await supabase
      .from("journey_tasks")
      .select("*")
    ...
    console.error("❌ getJourneyTasksByMonth error:", error);
    return [];
  }
}

// Get task by month number and task key (taskKey is the task id from journey-db.js, we match by task_name)
async function getJourneyTaskByKey(monthNumber, taskKey) {
  try {
    ...
    if (monthError || !month) {
      console.error("❌ Month not found:", monthNumber);
      return null;
    }
    ...
    if (!tasks || tasks.length === 0) {
      console.error("❌ No tasks found for month:", monthNumber);
      return null;
    }

    // IMPROVED: Better task matching
    // Normalize the taskKey for comparison
    const normalizedKey = taskKey.toLowerCase().replace(/_/g, " ");

    // Find exact match or fuzzy match
    const task = tasks.find((t) => {
      const taskNameNormalized = (t.task_name || "").toLowerCase();

      // Try exact key match in task name
      if (taskNameNormalized.includes(normalizedKey)) {
        return true;
      }
      ...
      const matchCount = keyWords.filter((word) =>
        taskNameNormalized.includes(word)
      ).length;
    ...
    if (!task) {
      console.warn(
        `⚠️ No task found for key "${taskKey}" in month ${monthNumber}`
      );
      console.warn(
        "Available tasks:",
    ...
    console.error("❌ getJourneyTaskByKey error:", error);
    return null;
  }
}

// Get ambassador's current month
async function getAmbassadorCurrentMonth(ambassadorId) {
  try {
    const { data, error } = await supabase
    ...
    console.error("❌ getAmbassadorCurrentMonth error:", error);
    return null;
  }
}

// Initialize or update ambassador's month progress
async function upsertAmbassadorMonthProgress(
  ambassadorId,
  monthId,
  isCurrent = false
) {
  try {
    // If setting a new current month, unset all other current months for this ambassador
    if (isCurrent) {
      await supabase
        .from("ambassador_journey_progress")
        .update({ current_month: false })
        .eq("ambassador_id", ambassadorId)
        .eq("current_month", true);
    }
      ...
      .upsert(
        {
          ambassador_id: ambassadorId,
          month_id: monthId,
          current_month: isCurrent,
          started_at: isCurrent ? new Date().toISOString() : undefined,
          updated_at: new Date().toISOString(),
        },
        {
          onConflict: "ambassador_id,month_id",
        }
      )
      .select()
      .single();
    ...
    console.error("❌ upsertAmbassadorMonthProgress error:", error);
    throw error;
  }
}

// Get all task completions for an ambassador
async function getAmbassadorTaskCompletions(ambassadorId) {
  try {
    const { data, error } = await supabase
      ...
      .select(
        ...
        *,
        journey_tasks!inner(
          ...
          task_name,
          month_id,
          journey_months!inner(month_number)
        )
      `
      )
      .eq("ambassador_id", ambassadorId)
    ...
    if (error) {
      console.error("❌ getAmbassadorTaskCompletions query error:", error);
      throw error;
    }
    return data || [];
  } catch (error) {
    console.error("❌ getAmbassadorTaskCompletions error:", error);
    return [];
  }
}

// Update or create task completion
async function upsertTaskCompletion(
  ambassadorId,
  taskId,
  status,
  notes = null
  ...
  try {
    // Get the task to find its month_id
    ...
    // If this is the first task for this month and no current month is set, set it
    ...
    if (!currentMonthCheck) {
      // Set this month as current if no current month exists
      await supabase
        .from("ambassador_journey_progress")
        .update({ current_month: false })
        .eq("ambassador_id", ambassadorId);
    ...
    if (status === "completed" && !completionData.completed_at) {
      completionData.completed_at = new Date().toISOString();
    }
    if (status === "in_progress" && !completionData.started_at) {
    ...
    const { data, error } = await supabase
      ...
      .upsert(completionData, {
        onConflict: "ambassador_id,task_id",
      })
      .select()
      .single();
    ...
    console.error("❌ upsertTaskCompletion error:", error);
    throw error;
  }
}
